...
async function getAllUsers(){
 ...
 userContainer.innerHTML +=
        ...
        <input  class="edit-${user.id}-user-name" id="editName"  placeholder="enter your name"/>
        <input   class="edit-${user.id}-user-avatar" id="editAvatar" placeholder="enter your pic url"/>
        <button onclick="saveUser(${user.id})">save</button>
        </div>
        </div>
        </div>`
       )
    //    value =${user.name}value = ${user.avatar}
}
getAllUsers()
//delete user
...
async function saveUser(userId){
    ...
    const editAvatar = document.querySelector("#editAvatar").value;
     console.log(userId)
